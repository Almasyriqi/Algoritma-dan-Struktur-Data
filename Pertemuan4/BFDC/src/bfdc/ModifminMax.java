 ...
 * NIM      : 1941720057
 */
public class ModifminMax {
    public int nilaiArray; 
    
    public static void hitungMinMax(int arr[], int min, int max, ModifMaxmin hasilBF) {
        for (int i = 0; i < 5; i++) // O(n)
        {
            if (arr[i]<min){  // O(1)
                min = arr[i]; // O(1)
            }
            else if (arr[i]>max){ // O(1)
                max = arr[i]; // O(1)
            }
        }
        hasilBF.maximum = max; // O(1)
        hasilBF.minimum = min; // O(1)
    }
